...
contract SwapAndBridgeOptimismRouter is Ownable {
    ...
    error SwapAndBridgeOptimismRouter__CallerNotManager();
    error SwapAndBridgeOptimismRouter__TokenCannotBeBridged();
        ...
        if (settings.bridgeTokens) {
            ...
            if (!l1TokenToBridge.isAddressZero()) {
                ...
                if (l2Token == address(0))
                    revert SwapAndBridgeOptimismRouter__TokenCannotBeBridged();
            }
        }
    ...
    function unlockCallback(
        ...
        if (msg.sender != address(manager))
            revert SwapAndBridgeOptimismRouter__CallerNotManager();
